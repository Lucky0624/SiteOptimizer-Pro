...
const tabKeys: { key: FilterTab; labelKey: string }[] = [
  { key: 'all', labelKey: 'tasks.all' }, { key: 'pending', labelKey: 'tasks.pending' },
  { key: 'running', labelKey: 'tasks.running' }, { key: 'completed', labelKey: 'tasks.completed' },
  { key: 'failed', labelKey: 'tasks.failed' }, { key: 'deferred', labelKey: 'tasks.deferred' },
]
...
export default function TaskQueue() {
  ...
  const { data: stats, refetch: refetchStats } = useApi(
    () => api.tasks.getStats(selectedWebsiteId ?? undefined),
    [selectedWebsiteId]
  )
  const { data, loading, refetch } = useApi(
    ...
    [page, activeTab, selectedWebsiteId]
  )
  ...
  const handleRetry = async (id: number) => {
    try { await api.tasks.retry(id); refetch(); refetchStats(); showToast(t('tasks.retrySuccess'), 'success') }
    catch (err: any) { showToast(err.message || t('tasks.retryError'), 'error') }
  }

  const handleAllocate = async () => {
    try { const r = await api.tasks.allocate(selectedWebsiteId ?? undefined); refetch(); refetchStats(); showToast(`${t('tasks.allocateSuccess')}: ${r.tasks_created}`, 'success') }
    catch (err: any) { showToast(err.message || t('tasks.allocateError'), 'error') }
  }

  const handleProcess = async () => {
    try { const r = await api.tasks.process(selectedWebsiteId ?? undefined); refetch(); refetchStats(); showToast(`${t('tasks.processSuccess')}: ${r.tasks_processed}`, 'success') }
    catch (err: any) { showToast(err.message || t('tasks.processError'), 'error') }
  }
  ...
  const getLabel = (taskType: string) => { const k = `task.${taskType}`; const v = t(k); return v === k ? taskType : v }

  return (
    <div className="space-y-6">
      <div className="flex justify-end">
        <WebsiteSelector value={selectedWebsiteId} onChange={id => { setSelectedWebsiteId(id); setPage(1) }} />
      </div>
        ...
        <div className="flex gap-1 rounded-xl p-1" style={{ border: '1px solid var(--border-color)', backgroundColor: 'var(--bg-input)' }}>
          {tabKeys.map((tab) => (
            <button key={tab.key} onClick={() => { setActiveTab(tab.key); setPage(1) }}
              ...
              style={activeTab === tab.key ? { backgroundColor: 'rgba(0,240,255,0.15)', color: 'var(--accent-cyan)', boxShadow: '0 0 10px rgba(0,200,230,0.2)' } : { color: 'var(--text-secondary)' }}>
              {t(tab.labelKey)}
            </button>
          ))}
        </div>
          ...
          <table className="w-full">
            <thead>
              <tr style={{ borderBottom: '1px solid var(--border-color)' }} className="text-left text-xs theme-text-secondary">
                {['tasks.taskId','tasks.type','tasks.priority','tasks.status','tasks.scheduled','tasks.actions'].map(k => (
                  <th key={k} className="px-4 py-3 font-medium">{t(k)}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {data?.items.map((task: TaskItem) => (
                <tr key={task.id} style={{ borderBottom: '1px solid var(--border-color)' }} className="transition-colors hover:opacity-90">
                  <td className="px-4 py-3"><p className="text-sm theme-text-primary">#{task.id}</p><p className="text-xs theme-text-secondary">URL: {task.url_id}</p></td>
                  <td className="px-4 py-3"><StatusBadge status={task.task_type} type="tag" label={getLabel(task.task_type)} /></td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {(task.status === 'failed' || task.status === 'deferred') && (
                        <button onClick={() => handleRetry(task.id)} className="rounded-lg p-1.5 theme-text-secondary hover:opacity-80" title={t('tasks.retry')}><RotateCcw size={14} /></button>
                      )}
                      {task.result_summary && (
                        <button onClick={() => setResultModal(task)} className="rounded-lg p-1.5 theme-text-secondary hover:opacity-80" title={t('tasks.viewResult')}><Eye size={14} /></button>
                      )}
                    </div>
                  </td>
                </tr>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm" onClick={() => setResultModal(null)}>
          <GlassCard className="w-full max-w-lg p-6" onClick={e => e.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              ...
              <button onClick={() => setResultModal(null)} className="theme-text-secondary">✕</button>
            </div>
